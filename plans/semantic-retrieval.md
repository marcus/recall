...
1. **Line ranges: present.** Every JSON result carries `line` (span start) and
   ...
   The raw JSON these findings were read off is committed rather than quoted
   here, recorded from the real CLI over a synthetic corpus:
   `cmd/recall-qmd/conformance/search-ranked/fixture/search-hybrid.json` for a
   result with a full `--explain` trace,
   `.../search-vector-similarity/fixture/vector-paraphrase.json` for a `vsearch`
   result and its cosine, `.../search-bm25-abstains/fixture/search-empty.json`
   for the empty answer, and `.../search-invalid-response/fixture/progress.txt`
   for the cold-start output that corrupts `--json`. They are regenerated by
   `cmd/recall-qmd/conformance/record-qmd-fixtures.sh`, so the sample cannot
   drift from what the adapter is tested against.
2. **Identity: file path per collection** (`qmd://clara-home/<relpath>`),
   distinct from `docid` (content hash). `source_record_id` = relpath, the same
   value the docs adapter derives for the same file.

   The consequence drawn here was wrong and is recorded so (td-396588, F1 on
   td-09f45f): identical `source_record_id` does NOT make cross-source
   corroboration collapse "work by construction". A lineage root is
   `<source_uid>:<local>`, so two sources never share one, and the
   record-identity key is scoped by source by design. What the parity actually
   enables is the location-scoped rule added in ranking to fix td-396588: two
   sources whose resolved location and record id agree are two views of one
   record. The parity was necessary and not sufficient, and assuming otherwise is
   why the inflation shipped.
...
5. **Index obligations:** indexing only via `recall/refresh` (`qmd update` +
   ...
   `index_config` = {qmd version, model names, mode}; generation reporting
   best-effort and declared.

   **Amended during implementation (td-09f45f).** The watermark is NOT
   index.sqlite's mtime. `docs/writing-an-adapter.md` names an mtime as a bad
   watermark twice — it is machine-local and unstable across hosts, and a
   committed conformance transcript would have carried it. What ships is counts
   derived from qmd's own reports: `collection=<name> files=<n>` for the corpus
   and `docs=<n> vectors=<n>` for the index. Both are stable across a rebuild of
   an unchanged corpus and identical on two machines indexing one corpus; both
   miss a same-count, same-size content replacement, which is declared in
   `doc.go` and in the freshness policy. `index_generation` is a digest over
   those counts plus `index_config`.
6. **Failure taxonomy:** missing binary/runtime → unreachable; non-JSON or
   pre-JSON noise on stdout → broken contract; timeout → deadline_exceeded. All
   degrade coverage (exit 3), source named.

   **Two spellings corrected during implementation (td-09f45f), because both
   named reasons the code cannot produce.** `spawn_failed` is what the core
   reports when it cannot start an ADAPTER process; a qmd binary this adapter
   cannot exec is a source it cannot reach, and `adapter.Classify` maps that to
   `unreachable`. And `invalid_response` is not in the reason vocabulary at all:
   a broken contract is a plain error, so it classifies as `adapter_error` and
   the words "invalid_response" appear only in the error message, where a person
   reads them.
