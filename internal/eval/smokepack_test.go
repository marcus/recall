...
func TestSmokeDebugDocsOrdering(t *testing.T) {
	ctx := context.Background()
	pack := smokePackDir(t)
	open := func(corpus string, settings map[string]any) *docs.Adapter {
		a := docs.New()
		if _, err := a.Initialize(ctx, adapter.Config{
			ProtocolVersionMin: 1, ProtocolVersionMax: 1, Workdir: t.TempDir(),
			SourceID: corpus, Location: filepath.Join(pack, "sources", corpus), Settings: settings,
		}); err != nil {
			t.Fatal(err)
		}
		return a
	}
	notes := open("notes", map[string]any{"extensions": []any{".md"}, "aliases": map[string]any{
		"projects/recall/decisions.md": []any{"ADR log"},
		"runbooks/backup-restore.md":   []any{"restore drill"}}})
	handbook := open("handbook", map[string]any{"extensions": []any{".md"}})
	for _, q := range []string{
		"projects/recall/architecture.md", "backup-restore.md",
		"the backup restore runbook for atlas storage", "ADR log",
		"a log of the ADRs we accepted", "how are results from different systems merged",
		"what do we do after we lose data", "how does recall decide two records are the same record",
		"quarterly revenue by region", "corroboration counts independent units",
		"lineage root definition", "restore drill", "retention of snapshots",
		"lineage grouping", "Atlas vendor contract renewal", "restore drill schedule rotation",
		"payroll restore drill vendor renewal", "Recall lineage design note",
		"restore drill rotation", "stream rotation", "restore drill dashboard",
		"restore drill rotation published",
	} {
		t.Logf("=== %s", q)
		for _, a := range []*docs.Adapter{notes, handbook} {
			resp, err := a.Search(ctx, recall.SearchRequest{Query: q, Limit: 4, Deadline: time.Now().Add(time.Minute)})
			if err != nil {
				t.Fatal(err)
			}
			for i, c := range resp.Candidates {
				t.Logf("   %d %s %v", i+1, c.Locator.Local, c.MatchSignals)
			}
		}
	}
}
