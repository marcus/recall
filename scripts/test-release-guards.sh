...
# Make must export RELEASE_VERSION as data. It must never interpolate an
# untrusted command-line value into shell source.
sentinel="$temporary/version-injection-ran"
malicious_version="v0.1.0\"; touch $sentinel; : #"
if make -s -C "$repo_root" check-release-state \
  "RELEASE_VERSION=$malicious_version" >/dev/null 2>&1; then
...
(
  ...
  RELEASE_VERSION=v0.1.0 ./scripts/check-release-state.sh pre-tag >/dev/null
  git -c user.name=release-test -c user.email=release-test@example.invalid \
