...
source_repo=${RECALL_SOURCE_REPO:-marcus/recall}
tap_repo=${RECALL_TAP_REPO:-marcus/homebrew-tap}
release_workflow=${RECALL_RELEASE_WORKFLOW:-release.yml}
discovery_timeout=${RECALL_RELEASE_DISCOVERY_TIMEOUT:-600}
poll_interval=${RECALL_RELEASE_POLL_INTERVAL:-5}
...
tap_repo_url() {
  if [[ -n ${RECALL_TAP_REPO_URL:-} ]]; then
    echo "$RECALL_TAP_REPO_URL"
    return
  fi
...
check_prerequisites() {
  ...
  [[ $discovery_timeout =~ ^[1-9][0-9]*$ ]] ||
    die "RECALL_RELEASE_DISCOVERY_TIMEOUT must be a positive integer"
  [[ $poll_interval =~ ^[1-9][0-9]*$ ]] ||
    die "RECALL_RELEASE_POLL_INTERVAL must be a positive integer"
...
validate_formula() {
  ...
  local validation_tap="recall-release/validation-$$-$RANDOM"
  ...
  (
    ...
    cp "$formula" "$validation_repo/Formula/recall.rb"
    brew style --formula "$validation_tap/recall"
    brew audit --strict --online --formula "$validation_tap/recall"
  )
}
...
push_tap_commit() {
  ...
  for attempt in 1 2 3; do
    ...
    if ! cmp -s "$tap_dir/Formula/recall.rb" "$expected_formula"; then
...
publish_tap() (
  ...
  mkdir "$formula_dir"
  expected_formula="$formula_dir/recall.rb"
  ...
  tap_formula="$tap_dir/Formula/recall.rb"
  [[ -f $tap_formula ]] || die "tap does not contain Formula/recall.rb"
  ...
  if [[ $transition_status == 10 ]]; then
    ...
    git -C "$tap_dir" add Formula/recall.rb
    git -C "$tap_dir" diff --cached --check
    git -C "$tap_dir" commit -m "recall $release_version"
    push_tap_commit "$tap_dir" "$expected_formula"
  fi

  git -C "$tap_dir" fetch origin main
  remote_commit=$(git -C "$tap_dir" rev-parse FETCH_HEAD)
  remote_formula="$temporary/remote-recall.rb"
  git -C "$tap_dir" show "$remote_commit:Formula/recall.rb" >"$remote_formula"
  ...
  echo "published and verified $tap_repo@$remote_commit: recall $remote_version ($remote_sha)"
)
