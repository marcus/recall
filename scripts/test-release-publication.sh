...
"$repo_root/scripts/render-homebrew-formula.sh" \
  v1.2.3 "$sha" "$temporary/expected/recall.rb" >/dev/null
"$repo_root/scripts/render-homebrew-formula.sh" \
  v1.2.2 "$sha" "$temporary/current/recall.rb" >/dev/null
check_formula_transition \
  "$temporary/current/recall.rb" "$temporary/expected/recall.rb" v1.2.3 ||
  fail "rejected an upgrade"

cp "$temporary/expected/recall.rb" "$temporary/current/recall.rb"
set +e
check_formula_transition \
  "$temporary/current/recall.rb" "$temporary/expected/recall.rb" v1.2.3
status=$?
...
sed -i.bak "s/$sha/1111111111111111111111111111111111111111111111111111111111111111/" \
  "$temporary/current/recall.rb"
rm "$temporary/current/recall.rb.bak"
...
"$repo_root/scripts/render-homebrew-formula.sh" \
  v1.2.4 "$sha" "$temporary/current/recall.rb" >/dev/null
if check_formula_transition \
  "$temporary/current/recall.rb" "$temporary/expected/recall.rb" v1.2.3 \
  >/dev/null 2>&1; then
...
cp "$temporary/current/recall.rb" "$temporary/seed/Formula/recall.rb"
git -C "$temporary/seed" add Formula/recall.rb
...
cp "$temporary/expected/recall.rb" "$temporary/publisher/Formula/recall.rb"
git -C "$temporary/publisher" add Formula/recall.rb
...
push_tap_commit "$temporary/publisher" "$temporary/expected/recall.rb"
git --git-dir="$temporary/tap.git" show main:Formula/recall.rb \
  >"$temporary/remote-formula.rb"
cmp -s "$temporary/remote-formula.rb" "$temporary/expected/recall.rb" ||
